...
class HierarchicalPolicyController:
    ...
    def __init__(
        ...
        *,
        ...
        self.fixed_sched_alpha = (
            None
            if fixed_sched_alpha is None
            else float(fixed_sched_alpha)
        )
    ...
    def _load_hrl_checkpoint(self, model_path: str) -> None:
        ...
        state_dict, migrated_tensors = (
            policy.migrate_action_space_checkpoint_tensors(model, state_dict)
        )
        if migrated_tensors:
            log(
    ...
    def predict(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        if (
            canonical_phase(state.get("request_type")) == DEC_PHASE
            and not self._decomposition_allowed(state)
        ):
            return {
                "action": "skip",
                "stop": True,
    ...
    def _predict_hrl(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        if request_type == DEC_PHASE:
            ...
            apply_allowed = (
                self._decomposition_allowed(state)
                and base_rels > 2
                and remaining > 0
            )
            ...
            if self.fixed_sched_alpha is None:
                ...
                sched_idx = min(
                    ...
                    key=lambda index: abs(
                        hrl.SCHED_ALPHA_VALUES[index] - alpha
                    ),
                )
            return {
                "action": SCHED_PHASE,
        ...
        if request_type == ENUM_PHASE:
            ...
            mask = [1.0] + [1.0 if enum_feasible else 0.0] * (
                len(hrl.ENUM_LABELS) - 1
            )
            mask = hrl.apply_action_ablation_mask(
            ...
            with self._torch.no_grad():
                ...
                predicted_value = float(
                    model.enum_critic(encoded).squeeze().item()
                )
    ...
    *,
    ...
    if wire_phase == "high":
        ...
        legacy_fields.extend(
            (f"search_strategy={strategy}", f"search_k={search_k}")
        )
    elif wire_phase == "low":
        wire_action = "low"
