...
def _mixed_masked_categorical(
    ...
    mixed_probs = (
        (1.0 - exploration_epsilons) * base_probs
        + exploration_epsilons * valid_probs
    )
    if coverage_mixes is not None:
        ...
        mixed_probs = (
            (1.0 - coverage_mixes) * mixed_probs
            + coverage_mixes * coverage_probs
        )
...
def ppo_update(
    ...
    if count == 0:
        if diagnostics is not None:
            diagnostics.update(
                {"transitions": 0, "updates": 0, "total_loss": 0.0}
            )
        return 0.0
    ...
    for _ in range(n_epochs):
        ...
        for start in range(0, count, batch_size):
            ...
            clip_fraction = (
                (torch.abs(unclamped_ratio - 1.0) > clip_eps).float().mean()
            )
