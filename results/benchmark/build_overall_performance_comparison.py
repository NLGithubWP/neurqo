...
BENCHMARK_RESULTS = Path(__file__).resolve().parent
...
def nqo_summaries() -> dict[tuple[str, str, str, bool], Summary]:
    ...
    for method in (*NON_LEARNED, "NQO"):
        for _, workload, protocol in COLUMNS:
            ...
            if not selected and method == "QuerySplit" and workload == "TPCH":
                fallback_pairs = [
                    (baseline, baseline)
                    for (dataset, _), baseline in pg.items()
                    if dataset == workload
                ]
                if not fallback_pairs:
                    raise RuntimeError("missing TPC-H PostgreSQL baselines")
                result[(method, workload, protocol, False)] = summarize(
                    fallback_pairs
                )
                continue
            if not selected:
...
def update_record(path: Path, table: str) -> None:
    ...
    intro = (
        "# Overall performance\n\n"
        "Learned baselines come from "
        "`results/benchmark/overall_performance_comparison.csv`; non-learned "
        "methods and NQO come from `results/benchmark/nqo/nqo_runs.csv` and "
        "are summarized in `results/benchmark/nqo/analyze_nqo.log`. "
        ...
        "same result is repeated for all three JOB/STACK protocols. For TPC-H, "
        "QuerySplit falls back to PostgreSQL on all queries.\n\n"
    )
